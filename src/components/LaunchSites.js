...
function LaunchSites({ data, fetchData, findLinkName }) {
	useEffect(() => {
		console.log('LaunchSites page intiliazed');
	...
	return (
        ...
        <div className="row justify-content-center g-1 mt-1">
							<Main
							    name={`${item.name}${item.full_name ? ' (' + item.full_name + ')': ''}`}
				...
				<div className="row justify-content-between gx-1 mt-1">
					<div className="col-md-6 flex-fill">
						<div className="row justify-content-evenly gx-1 align-items-stretch ">
							...
							<div className="col-4 flex-fill">
								<Data data={item.launch_attempts} name="Launch attempts" />
							</div>
							<div className="col-4 flex-fill">
								<Data data={item.launch_successes} name="Launch successes" />
							</div>
						</div>
